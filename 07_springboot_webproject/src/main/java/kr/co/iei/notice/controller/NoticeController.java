...
public class NoticeController {
    ...
    public String write(Notice n,MultipartFile[] upfile,Model model) {
        ...
        //화면에서 multiple옵션으로 첨부파일 여러개를 첨부하게 했으므로 배열로 처리
        //input type=text에 아무것도 입력하지 않고 submit을 하면 null이 아니라 빈문자열("")
        //input type=file에 아무것도 첨부하지 않고 submit을 하면 배열의 길이가 0이 아니라, 1이고, 첫번째 파일이 비어있음
        
        //첨부파일은 서버에 업로드를 수행하고, 업로드 된 결과(업로드한 파일이름)를 db 저장
        //notice_file에 insert하기 위한 데이터를 생성
        ...
        if(!upfile[0].isEmpty()) {
            ...
            for(MultipartFile file : upfile) {
                //사용자가 업로드한 파일 이름 출력
                String filename = file.getOriginalFilename(); // ice5.png
    ...
    public String search(String type,String keyword,int reqPage,Model model) {
        ...
        NoticeListData nld = noticeService.selectNoticeList(type,keyword,reqPage);
        model.addAttribute("list",nld.getList());
        model.addAttribute("pageNavi",nld.getPageNavi());
    ...
    public String delete(int noticeNo,Model model) {
        //삭제를 하게 되면 해당 게시글의 첨부파일도 삭제 -> 삭제결과로 파일 목록을 가져옴
    ...
    public String updateComment(NoticeComment nc,Model model) {
        ...
        if(result>0) {
            ...
            model.addAttribute("msg","댓글이 수정 실패 했습니다.");
            model.addAttribute("icon","error");
        }
        model.addAttribute("loc","/notice/view?check=1&noticeNo="+nc.getNoticeRef());
        return "common/msg";
    ...
    public String deleteComment(NoticeComment nc,Model model) {
        ...
        if(result>0) {
            model.addAttribute("title","성공");
            model.addAttribute("msg","댓글을 삭제하셨습니다.");
            model.addAttribute("icon","success");
    ...
    public int likePush(int noticeCommentNo, int isLike, @SessionAttribute(required = false) Member member) {
    	//@SesstionAttribute에서 로그인정보를 가지고올 때 required옵션을 명시하지 않으면 기본적으로 true
    	// -> 로그인이 되어 있지 않으면 에러가 발생
    	// -> 로그인이 되어있지 않은 상태에서 에러를 발생시키지 않으려면 (required = false)옵션을 추가
    	// 		-> 로그인이 되어있으면 로그인 한 회원정보/로그인이 되어있지 않으면 null
    	if(member == null) {
    		return -10; //로그인이 안되어있다 신호주자
    	}else {    		
