...
public class MemberController {
	...
	public String join(Member m, Model model, HttpSession session) {
		...
		//조회조건에 아이디가 포함되어있으므로 조회결과는 회원1명 또는 0명 0> Member타입으로 결과 받기
		Member member = memberService.selectOneMember(m);
		//System.out.println(member); 출력 확인용
		if(member == null) {
			//아이디 패스워드 일치하지 않는 회원이 있으면  null
			...
			if(member.getMemberLevel() == 3) {
				...
				// -> 메인페이지
				//model데이터를 등록하면 해당 요청이 끝나면 등록한 데이터를 더이상 사용할 수 없음
				//model.addAttribute("member", member);
				//데이터를 계속 저장해주기 위해서 request가 아니라 session이라는 공간에 데이터를 저장
				// -> session영역을 가져오는 방법을 request.getSession();
				//HttpSession session = request.getSession();
				//세션에 member라는 이름으로 로그인한 회원 정보를 저장
				session.setAttribute("member", member);
				return "redirect:/";
				//session 쌓이고 쌓이면 서버가 기억할 데이터가 커서 과부하가 걸림
			}
		}
	}
	...
	public String checkId(String checkId, Model model) {
		Member member = memberService.selectOneMember(checkId);//메소드 오버로딩
		if(member == null) {
	...
	//세션에 저장되어 있는 정보를 컨트롤러에서 가져와서 회원정보를 그때 다시 조회해서 화면으로 주는 방식
	@GetMapping(value="/mypage1")
	public String mypage1(@SessionAttribute Member member, Model model) {//세션에 저장되어 컨트롤러 사용하겠다, 저장되어 있는 키값 가지고 와
		String memberId = member.getMemberId();
		Member m = memberService.selectOneMember(memberId);//member 와 m 의 데이터 항상 일치하는건 아니다 (현재는 일치)
	...
	//session 에 있는 로그인 정보와 db에 변경된 정보와 다르다. 이 방법이 좀더 정확하다
