...
public class NoticeDao {
    ...
    public int selectNoticeTotalCount() {
        ...
        //조회결과가 단일값(1행 1열)인 경우에만 사용할 수 있는 메소드
        //쿼리문 수행겨로가를 두번째 매개변수타입으로 변환해서 바로 리턴
    ...
    public int selectNoticeTotalCount(String type,String keyword) {
    ...
    public List selectNoticeList(String type, String keyword,int start,int end) {
        String query = null;
        if(type.equals("title")) {
            query = "select * from (select rownum as rnum , n.* from (select * from notice where notice_title like '%'||?||'%' order by 1 desc)n) where (rnum between ? and ?)";
        }else if(type.equals("writer")) {
            query = "select * from (select rownum as rnum , n.* from (select * from notice where notice_writer like '%'||?||'%' order by 1 desc)n) where (rnum between ? and ?)";
        }
