...
public class NoticeService {
    ...
    public int insertNotice(Notice n, List<NoticeFile> fileList) {
        ...
        if(result >0) {
            ...
            for(NoticeFile noticeFile : fileList) {
                //1번작업으로 insert될때 생성된 공지사항번호를 저장한 후 notice_file insert 요청
    ...
    public NoticeListData selectNoticeList(String type, String keyword,int reqPage) {
        //reqPage : 사용자가 요청한 페이지 번호
                //한 페이지당 보여줄 게시물의 수(지정)   -> 10개
                int numPerPage = 10;
                ...
                int start = end -numPerPage + 1;
                //요청페이지에 필요한 공지사항 목록을 조회
                List list = noticeDao.selectNoticeList(type,keyword,start,end);
                //페이지 네비게이션 제작(사용자가 클릭해서 다른 페이지를 요청할 수 있도록 하는 요소 ) 제작
                //페이지 네비게이션을 서비스에서 만드는 이유 -> 총 게시물수를 조회해와야 가능하기 떄문
                //select count(*) from notice;
                int totalCount = noticeDao.selectNoticeTotalCount(type,keyword);
                //전체 페이지 수 계산
                int totalPage = 0;
                if(totalCount%numPerPage ==0) {
                    totalPage = totalCount/numPerPage;
                ...
                if(pageNo != 1) {
                    pageNavi += "<li>";
                    pageNavi += "<a class ='page-item ' href='/notice/search?type="+type+"&keyword="+keyword+"&reqPage="+(pageNo-1)+"'>";
                    pageNavi += "<span class = 'material-icons'>chevron_left</span>";
                    pageNavi += "</a></li>";
                }
                ...
                for(int i =0;i<pageNaviSize;i++) {
                    pageNavi += "<li>";
                    if(pageNo==reqPage) {
                        pageNavi += "<a class ='page-item active-page' href='/notice/search?type="+type+"&keyword="+keyword+"&reqPage="+pageNo+"'>";
                        
                    }else {
                        pageNavi += "<a class ='page-item' href='/notice/search?type="+type+"&keyword="+keyword+"&reqPage="+pageNo+"'>";
    ...
    public List<NoticeFile> deleteNotice(int noticeNo) {
        //Notice테이블에서 공지사항 삭제/NoticeFile에서 해당 공지사항의 첨부파일 삭제/NoticeFile에서 해당공지사항의 첨부파일 조회
    ...
    public List<NoticeFile> updateNotice(Notice n, List<NoticeFile> fileList, int[] delFileNo) {
        ...
        //notice업데이트, notice_file insert(추가한 파일이 있을때만), notice_file delete(삭제한 파일이 있을때만)
        int result = noticeDao.updateNotice(n);
        if(result >0) {
            //추가한 파일이 있는 경우 추가 파일 insert
            for(NoticeFile noticeFile : fileList) {
                result += noticeDao.insertNoticeFile(noticeFile);
            }
            //삭제한 파일이 있는경우 (파일을 DB에서 조회한 후 (실제 폴더에서도 지우기 위함 ) -> 삭제)
            if(delFileNo != null) {
	...
	public int likePush(int noticeCommentNo, int isLike, int memberNo) {
		...
		if(result >0) {
			//좋아요, 좋아요취소 로직을 수행하고나면 현재 좋아요 갯수를 조회해서 리턴
			int likeCount = noticeDao.selectNoticeCommentLikeCount(noticeCommentNo);
			return likeCount;
		}else {
			return -1;
